...
# ============================================================
# #12 LTX-2.3 + 10Eros v1.5 + Licon MSR V2 (COMPLETE v4)
...
#   * v4 adds ComfyUI_essentials required by the published MSR V2 workflow.
#   * v4 clears stale port 8188 listeners before smoke-testing the new server.
#   * v4 seeds PromptRelay local_prompts so first Queue cannot fail on an empty field.
...
LOG="${LOGDIR}/setup_12_ltx23_10eros_msr_complete_v4_${STAMP}.log"

mkdir -p "$ROOT" "$LOGDIR"
exec > >(tee -a "$LOG") 2>&1

echo "============================================================"
echo "[#12 COMPLETE v4] LTX-2.3 + 10Eros v1.5 + Licon MSR V2"
echo "Time: $(date -Is)"
echo "Root: $ROOT"
echo "ComfyUI: $COMFY"
echo "Port: $PORT"
...
# Required by the published MSR V2 sample workflow.
...
def patch_workflow(checkpoint_name, outname):
    ...
    for n in j.get("nodes", []):
        ...
        elif t == "PromptRelayEncode":
            # Current PromptRelay requires at least one non-empty local prompt.
            # Widget order is: global_prompt, local_prompts, segment_lengths, epsilon.
            while len(w) < 4:
                w.append("")
            if not str(w[1]).strip():
                w[1] = "Maintain the same scene and subject throughout the video."
            if w[2] is None:
                w[2] = ""
            if w[3] in (None, ""):
                w[3] = 0.001
...
readme = """#12 COMPLETE v4 LTX-2.3 + 10Eros v1.5 + Licon MSR V2
...
Before Queue:
  ...
  Prompt Relay Encode -> Local prompts is REQUIRED by the current node.
  v4 pre-fills it with:
    Maintain the same scene and subject throughout the video.
  Replace it with one or more local prompts separated by | when needed.
...
for fn in files:
    ...
    assert "PromptRelayEncode" in types, (fn, "missing PromptRelayEncode")
    assert len([n for n in j["nodes"] if n.get("type") == "LoadImage"]) >= 5
    relay_nodes = [n for n in j["nodes"] if n.get("type") == "PromptRelayEncode"]
    for rn in relay_nodes:
        rw = rn.get("widgets_values", [])
        assert len(rw) >= 2 and str(rw[1]).strip(), (fn, "PromptRelay local_prompts is empty")
    print("[WF OK]", fn)
...
# Clear stale ComfyUI processes/listeners before smoke test so /object_info
# cannot accidentally come from an older environment on port 8188.
...
SERVER_LOG="${LOGDIR}/comfy_12_ltx23_10eros_msr_complete_v4_${STAMP}.log"
...
if ! kill -0 "$PID" 2>/dev/null; then
  echo "[ERROR] The newly started #12 ComfyUI process exited; refusing to trust a stale /object_info response."
  tail -n 160 "$SERVER_LOG" || true
  exit 1
fi

if ! tr '\0' ' ' < "/proc/$PID/cmdline" | grep -q -- "--port $PORT"; then
  echo "[ERROR] PID $PID is not the expected #12 ComfyUI server."
  tr '\0' ' ' < "/proc/$PID/cmdline" || true
  exit 1
fi

echo "[OK] Verified new #12 ComfyUI PID $PID owns the intended launch command."

if ! grep -q "custom_nodes/ComfyUI_essentials" "$SERVER_LOG"; then
  echo "[ERROR] ComfyUI_essentials did not appear in the custom-node import log."
  tail -n 200 "$SERVER_LOG" || true
  exit 1
fi
echo "[OK] ComfyUI_essentials import confirmed."
...
echo
echo "============================================================"
echo "[SUCCESS] #12 COMPLETE v4 environment is up."
