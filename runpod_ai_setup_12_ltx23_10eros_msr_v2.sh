...
# ============================================================
# #12 LTX-2.3 + 10Eros v1.5 + Licon MSR V2 (v2)
...
#   - Official LTX-2.3 FP8 fallback workflow for infrastructure test
#
# Environment:
#   /workspace/runpod-slim/ComfyUI-LTX23-10Eros-MSR
#
# Notes:
#   * This is a candidate v1 until an actual Queue/output test passes.
#   * Licon MSR V2 is the multi-reference component.
#   * 10Eros + MSR is an experimental combination; an official LTX-2.3
#     fallback workflow is installed in the same environment.
# ============================================================
...
LOG="${LOGDIR}/setup_12_ltx23_10eros_msr_v2_${STAMP}.log"

mkdir -p "$ROOT" "$LOGDIR"
exec > >(tee -a "$LOG") 2>&1

echo "============================================================"
echo "[#12 v2] LTX-2.3 + 10Eros v1.5 + Licon MSR V2"
echo "Time: $(date -Is)"
echo "Root: $ROOT"
echo "ComfyUI: $COMFY"
echo "Port: $PORT"
...
echo "[6/9] Download primary 10Eros v1.5 + official LTX-2.3 FP8 fallback"
echo "[INFO] Workspace disk status before large downloads:"
df -h "$ROOT" || true
FREE_KB="$(df -Pk "$ROOT" | awk 'NR==2 {print $4}')"
# 10Eros BF16 ~46 GB + official FP8 fallback + working/cache headroom.
MIN_FREE_KB=$((85 * 1024 * 1024))
if [[ "${FREE_KB:-0}" -lt "$MIN_FREE_KB" ]]; then
  echo "[WARN] Less than ~85 GiB free on /workspace."
  echo "[WARN] The full v2 install (10Eros BF16 + official FP8 fallback) may not fit."
fi
# PRIMARY: 10Eros v1.5 BF16.
# Larger on disk, but avoids relying on experimental GGUF+LoRA behavior for MSR.
hf_get \
...
# FALLBACK: known LTX-2.3 checkpoint for validating the MSR stack itself.
hf_get \
  "Lightricks/LTX-2.3" \
...
readme = """#12 v2 LTX-2.3 + 10Eros v1.5 + Licon MSR V2

Recommended first test:
  12B_LTX23_OFFICIAL_FP8_MSR_V2_TEST.json
This validates ComfyUI + LTX-2.3 + Licon MSR V2.
...
Important:
  ...
  10Eros + MSR is a candidate combination and is not promoted to canonical
  until a successful Queue/output test confirms compatibility and quality.
...
required = [
    ...
    root/"models/checkpoints/ltx-2.3-22b-dev-fp8.safetensors",
...
SERVER_LOG="${LOGDIR}/comfy_12_ltx23_10eros_msr_v2_${STAMP}.log"
...
echo
echo "============================================================"
echo "[SUCCESS] #12 v2 environment is up."
echo "[INFO] Large-download cache/temp paths are fixed under /workspace."
echo
echo "Open RunPod proxy for port: $PORT"
echo
echo "FIRST workflow (stack validation):"
echo "  12B_LTX23_OFFICIAL_FP8_MSR_V2_TEST.json"
echo
echo "MAIN 10Eros workflow:"
