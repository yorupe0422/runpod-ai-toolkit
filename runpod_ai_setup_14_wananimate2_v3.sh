...
5) To rebuild with a different preset, rerun the setup script with e.g.:
   WAN_PRESET=quality-bf16 bash runpod_ai_setup_14_wananimate2_v3.sh
EOS

log "Done"
echo ""
echo "[INFO] Validate NVIDIA driver / PyTorch CUDA"
nvidia-smi
...
if not torch.cuda.is_available():
    print("[ERROR] PyTorch cannot initialize CUDA.", file=sys.stderr)
    sys.exit(1)
print("GPU              :", torch.cuda.get_device_name(0))
print("VRAM GiB         :", round(torch.cuda.get_device_properties(0).total_memory / 1024**3, 2))
print("[OK] CUDA validation passed.")
PY
