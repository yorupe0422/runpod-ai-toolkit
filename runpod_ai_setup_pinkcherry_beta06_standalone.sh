...
# PinkCherry beta-0.6 FL2VA — standalone, isolated RunPod setup.
...
on_error(){ local rc=$?; [[ $rc -eq 0 ]] || { red "SETUP FAILED (exit=$rc, line=${BASH_LINENO[0]:-unknown})"; echo "Log: $LOG"; }; exit "$rc"; }
trap on_error EXIT
...
ensure_support(){
  local rel="$1" url="$2" min="$3" target="$COMFY/models/$rel"
...
echo "============================================================"
echo " PinkCherry beta-0.6 FL2VA — STANDALONE SETUP"
echo "============================================================"

echo "[1/8] System tools"
if ! command -v git >/dev/null 2>&1 || ! command -v curl >/dev/null 2>&1 || \
   ! command -v aria2c >/dev/null 2>&1 || ! command -v lsof >/dev/null 2>&1; then
  apt-get update -qq
  DEBIAN_FRONTEND=noninteractive apt-get install -y -qq git curl ca-certificates aria2 lsof
fi

echo "[2/8] Verify beta-0.6 workflow source"
curl -fL --retry 5 "$WORKFLOW_URL?v=$(date +%s)" -o "$WORKFLOW_TMP"
...
if [[ -s "$DEST" ]] && [[ "$(stat -c%s "$DEST")" -ge "$MODEL_MIN_BYTES" ]] && \
  ...
  [[ "$(sha256sum "$DEST" | awk '{print $1}')" == "$MODEL_SHA256" ]] || die "Beta-0.6 SHA256 mismatch"
fi
