...
SCRIPT_VERSION="8.2.0-addons-nsfw-lockfix"
...
step "[7/11] Build five Studio add-on workflows"
...
WF_MULTI="$WF_MULTI" WF_NSFW="$WF_NSFW" "$PYTHON_BIN" - <<'PY'
import copy, json, os
...
def configure(variant):
    ...
    if variant in {"general_quality", "general_fast", "a2r", "nsfw"}:
...
for workflow in \
  "$WF_A2R" "$WF_ANYPOSE" "$WF_CLOTHES" "$WF_MULTI" "$WF_NSFW"; do
