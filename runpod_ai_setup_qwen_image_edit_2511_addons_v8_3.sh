...
SCRIPT_VERSION="8.3.0-addons-nsfw-preview"
...
LOG="$ROOT/qwen_image_edit_addons_setup.log"
...
MARKER="$COMFY/.qwen-image-edit-addons-managed"
...
NSFW_FILE="qwen-image-edit-plus-nsfw-lora.safetensors"
...
WF_NSFW="09_QWEN_NSFW_ADULT_EDIT_QUALITY_40STEP.json"
WF_NSFW_FAST="10_QWEN_NSFW_ADULT_EDIT_FAST_4STEP_PREVIEW.json"
...
step "QWEN IMAGE EDIT 2511 — STUDIO ADD-ONS"
...
step "[2/11] Verify BASE environment"
[[ -d "$COMFY/.git" && -f "$COMFY/main.py" ]] || \
  die "Qwen BASE environment was not found at $COMFY. Run the BASE installer first."
[[ -x "$PYTHON_BIN" ]] || die "Dedicated BASE Python was not found: $PYTHON_BIN"
git -C "$COMFY" cat-file -e "$COMFY_COMMIT^{commit}" 2>/dev/null || \
  die "BASE ComfyUI is not the required pinned build; rerun the BASE installer first"
...
ok "BASE environment found"
printf '%s\n' "version=$SCRIPT_VERSION" "base_comfy_commit=$COMFY_COMMIT" > "$MARKER"

step "[3/11] Verify BASE Python environment"
"$PYTHON_BIN" - <<'PY'
import torch
assert torch.cuda.is_available(), "CUDA is not available inside the BASE venv"
print(f"[OK] BASE PyTorch {torch.__version__}; CUDA {torch.version.cuda}")
PY

step "[4/11] Add-on model directories and disk preflight"
mkdir -p \
...
declare -a MODEL_PATHS=(
  ...
  "$COMFY/models/loras/$NSFW_FILE"
)
declare -a MODEL_SIZES=(
  613580128 295146208 295146216 590058864
)
declare -a MODEL_SHAS=(
  ...
  16c4841028615bb82c38e79756c0abad42494d85bca0daebc2939384a74d86bb
)
...
step "[5/11] Verified resumable add-on downloads (~1.8 GB including NSFW)"
...
download_hf "ScottzillaSystems/qwen-image-edit-plus-nsfw-lora" \
  "d47310899f31bfba27cc2287fe007728a3eda8a3" \
  "$NSFW_FILE" \
  "$COMFY/models/loras/$NSFW_FILE" \
  590058864 16c4841028615bb82c38e79756c0abad42494d85bca0daebc2939384a74d86bb
...
step "[7/11] Build six Studio add-on workflows"
...
WF_MULTI="$WF_MULTI" WF_NSFW="$WF_NSFW" WF_NSFW_FAST="$WF_NSFW_FAST" "$PYTHON_BIN" - <<'PY'
import copy, json, os
...
NSFW = "qwen-image-edit-plus-nsfw-lora.safetensors"
...
PROMPTS = {
    ...
    "nsfw": "nsfw. For fictional adults only. Apply only the requested adult-content edit to image 1 while preserving identity, adult appearance, facial features, body proportions, pose, camera angle, composition, background and lighting unless explicitly requested otherwise. Maintain realistic anatomy, coherent hands and clean image detail.",
    "nsfw_fast": "nsfw. For fictional adults only. Apply only the requested adult-content edit to image 1 while preserving identity, adult appearance, facial features, body proportions, pose, camera angle, composition, background and lighting unless explicitly requested otherwise. Maintain realistic anatomy, coherent hands and clean image detail.",
}
...
NOTES = {
    ...
    "nsfw": "# NSFW Adult Edit — QUALITY\nInput 1: clearly adult fictional subject only.\nMCNL NSFW LoRA at strength 0.8. Edit the prompt, then run in 40-step quality mode.\nNever use images depicting or ambiguously resembling minors.",
    "nsfw_fast": "# NSFW Adult Edit — FAST PREVIEW\nInput 1: clearly adult fictional subject only.\nMCNL NSFW LoRA at strength 0.75, then Lightning 4-step mode. Use only to test prompt, composition and LoRA strength; use the 40-step workflow for final output.\nNever use images depicting or ambiguously resembling minors.",
}
...
OUTPUT_PREFIX = {
    ...
    "nsfw": "Qwen_NSFW_Adult_Edit",
    "nsfw_fast": "Qwen_NSFW_Adult_Edit_Fast_Preview",
}
...
def add_nsfw_then_lightning(sg):
    lightning = node_by_id(sg["nodes"], 153)
    incoming_id = lightning["inputs"][0].get("link")
    if incoming_id is None:
        raise RuntimeError("Lightning LoRA has no model input")
    incoming = next(link for link in sg["links"] if link["id"] == incoming_id)

    nsfw = copy.deepcopy(lightning)
    nsfw["id"] = 197
    nsfw["pos"] = [nsfw["pos"][0], nsfw["pos"][1] - 160]
    nsfw["order"] = max(n.get("order", 0) for n in sg["nodes"]) + 1
    nsfw["title"] = "MCNL NSFW — preview strength 0.75"
    nsfw["inputs"][0]["link"] = incoming_id
    nsfw["outputs"][0]["links"] = [390]
    nsfw["widgets_values"] = [NSFW, 0.75]

    incoming["target_id"], incoming["target_slot"] = 197, 0
    lightning["inputs"][0]["link"] = 390
    sg["nodes"].append(nsfw)
    sg["links"].append({
        "id": 390, "origin_id": 197, "origin_slot": 0,
        "target_id": 153, "target_slot": 0, "type": "MODEL",
    })
    sg["state"]["lastNodeId"] = max(int(sg["state"].get("lastNodeId", 0)), 197)
    sg["state"]["lastLinkId"] = max(int(sg["state"].get("lastLinkId", 0)), 390)

def configure(variant):
    data = copy.deepcopy(template)
    fast = variant in {"general_fast", "person_fast", "anypose", "nsfw_fast"}
    ...
    for node in sg["nodes"]:
        ...
        if node_type == "PrimitiveBoolean" and node.get("id") == 168:
            ...
            if variant == "a2r":
                node["widgets_values"] = [A2R, 0.85]
            elif variant == "nsfw":
                node["widgets_values"] = [NSFW, 0.8]
            else:
                node["widgets_values"] = [LIGHTNING, 1.0]
        elif node_type == "TextEncodeQwenImageEditPlus" and node.get("id") == 151:
            node["widgets_values"] = [PROMPTS[variant]]

    if variant in {"a2r", "nsfw"}:
    ...
    if variant in {"general_quality", "general_fast", "a2r", "nsfw", "nsfw_fast"}:
...
def validate_workflow(data, variant):
    ...
    if variant == "a2r":
        ...
        expected = [LIGHTNING, NSFW]
    elif variant == "anypose":
        expected = [LIGHTNING, POSE_BASE, POSE_HELPER]
    else:
        expected = [LIGHTNING]
    if loras != expected:
        raise RuntimeError(f"{variant} LoRA chain mismatch: {loras}")
    turbo = node_by_id(sg["nodes"], 168)["widgets_values"][0]
    if turbo != (variant in {"general_fast", "person_fast", "anypose", "nsfw_fast"}):
...
outputs = [
    ...
    (os.environ["WF_NSFW"], "nsfw"),
    (os.environ["WF_NSFW_FAST"], "nsfw_fast"),
]
...
for workflow in \
  "$WF_A2R" "$WF_ANYPOSE" "$WF_CLOTHES" "$WF_MULTI" "$WF_NSFW" "$WF_NSFW_FAST"; do
...
print("[OK] Eight model files indexed")
PY
fi
...
echo
echo "================================================================="
echo " QWEN IMAGE EDIT 2511 STUDIO ADD-ONS READY"
echo "================================================================="
...
echo "NSFW trial        : $WF_NSFW_FAST (4-step preview; fictional adults only)"
echo "NSFW final        : $WF_NSFW (40-step quality; fictional adults only)"
echo "================================================================="
