...
# RunPod one-shot installer: independent Qwen-Image-Edit-2511 Base for ComfyUI.
# Target: NVIDIA RTX 5090 / 32 GB. No existing ComfyUI environment is required.

SCRIPT_VERSION="8.2.0-base-lockfix"
...
LOG="$ROOT/qwen_image_edit_base_setup.log"
...
MARKER="$COMFY/.qwen-image-edit-base-managed"
...
cleanup() {
  ...
  [[ -z "${ARIA_AUTH_CONFIG:-}" ]] || rm -f "$ARIA_AUTH_CONFIG" || true
...
step "QWEN IMAGE EDIT 2511 — BASE (FASTEST USABLE INSTALL)"
...
missing_tools=0
for cmd in git curl aria2c sha256sum stat df awk grep tee mktemp flock fuser; do
  command -v "$cmd" >/dev/null 2>&1 || missing_tools=1
done
if (( missing_tools )); then
  apt-get update
  DEBIAN_FRONTEND=noninteractive apt-get install -y \
    git curl aria2 ca-certificates coreutils procps util-linux psmisc python3-venv
fi
for cmd in git curl aria2c sha256sum stat df awk grep tee mktemp flock fuser; do
...
if [[ -d "$COMFY/.git" && ! -f "$COMFY/main.py" ]]; then
  preserved="$COMFY.no-checkout.$(date +%Y%m%d%H%M%S)"
  mv "$COMFY" "$preserved"
  warn "Interrupted v7 no-checkout repository preserved at $preserved"
fi
if [[ ! -d "$COMFY/.git" ]]; then
...
step "[4/11] Base model directories and disk preflight"
mkdir -p \
...
declare -a MODEL_SIZES=(
  20533762817 9384670680 253806246 849608296
)
...
step "[5/11] 4-way segmented, verified, resumable base-model downloads"
ARIA_AUTH_CONFIG="$(mktemp)"
chmod 600 "$ARIA_AUTH_CONFIG"
if [[ -n "${HF_TOKEN:-}" ]]; then
  printf 'header=Authorization: Bearer %s\n' "$HF_TOKEN" > "$ARIA_AUTH_CONFIG"
fi

download_hf() {
  local repo="$1" revision="$2" remote="$3" dest="$4" expected_size="$5" expected_sha="$6"
  local url part control attempt=1 delay=20 aria_code=0 connections=4
  ...
  echo "[INFO] aria2: 4 segments / 4 connections; interrupted downloads resume automatically"
  while (( attempt <= 7 )); do
    if (( attempt >= 4 )); then
      connections=1
    fi
    aria_code=0
    [[ ! -s "$part" ]] || \
      echo "[RESUME] $(basename "$dest") (attempt $attempt/7, connections=$connections)"
    aria2c \
      --conf-path="$ARIA_AUTH_CONFIG" \
      --continue=true \
    ...
    if (( aria_code == 0 )) && file_ok "$part" "$expected_size" "$expected_sha"; then
      mv -f "$part" "$dest"
      rm -f "$control"
    ...
    if (( aria_code == 0 )); then
      # aria2 declared completion, so a size/SHA mismatch means corrupt data.
      rm -f "$control"
    ...
    warn "Download interrupted or rate-limited (aria2=$aria_code). Partial data and piece map are preserved."
    if (( attempt == 3 )); then
      echo "[FALLBACK] Further retries use one connection to reduce Hugging Face rate-limit pressure."
    fi
    echo "[WAIT] Retry in ${delay}s; rerunning this SH later is safe."
    sleep "$delay"
...
step "[7/11] Build four core ready-to-use workflows"
...
for workflow in \
  "$WF_GENERAL_QUALITY" "$WF_GENERAL_FAST" "$WF_PERSON_QUALITY" "$WF_PERSON_FAST"; do
...
print("[OK] Four base model files indexed")
PY
fi
...
echo
echo "================================================================="
echo " QWEN IMAGE EDIT 2511 BASE READY"
echo "================================================================="
...
echo
echo "Optional Studio add-ons: Anything2Real + AnyPose can be installed later"
echo
