...
# Python packages contain thousands of small files. Writing those into
# /workspace (network storage) is often the slowest part of RunPod setup.
# Keep models/code/workflows persistent, but build the disposable venv on the
# container-local overlay. A Pod restart rebuilds only this small environment;
# verified multi-GB models remain under /workspace and are reused.
FAST_VENV="${FAST_VENV:-/tmp/comfyui-h3-venv-py312}"
PIP_CACHE_DIR="${PIP_CACHE_DIR:-/tmp/pip-cache-comfyui-h3}"
...
cleanup_on_error() {
  ...
  if [[ $rc -ne 0 ]]; then
    red ""
    red "SETUP #5 COMPLETE v4 FAST FAILED"
...
echo
echo "================================================================="
echo " SETUP #5 COMPLETE v4 FAST — MiniMax H3 / PinkCherry"
echo "================================================================="
...
# Also acquire the old V3 lock so V4 cannot start while V3 is still installing.
exec 8>"$LOCK_FILE"
flock -n 8 || die "SETUP #5 V3 is still running; stop it before starting V4 FAST"
LOCK_FILE="$BASE/.setup5-complete-v4-fast.lock"
exec 9>"$LOCK_FILE"
flock -n 9 || die "Another SETUP #5 V4 FAST installer is already running"
...
if [[ ! -d "$COMFY/.git" ]]; then
  rm -rf "$COMFY"
  git clone -q --depth 1 --branch "$COMFY_TAG" \
    https://github.com/Comfy-Org/ComfyUI.git "$COMFY"
fi

cd "$COMFY"
if ! git rev-parse -q --verify "refs/tags/$COMFY_TAG" >/dev/null; then
  git fetch -q --depth 1 origin "refs/tags/$COMFY_TAG:refs/tags/$COMFY_TAG"
fi
git reset --hard -q
...
# -----------------------------------------------------------------------------
echo "[3/10] FAST local Python environment"

# Remove only the old SETUP #5 venv. Models, workflows, inputs and outputs are
# outside this directory and are never touched here.
if [[ -L "$COMFY/.venv" ]]; then
  rm -f "$COMFY/.venv"
elif [[ -d "$COMFY/.venv" ]]; then
  yellow "  Removing slow /workspace venv from V3 (models are preserved)"
  rm -rf "$COMFY/.venv"
fi

if [[ ! -x "$FAST_VENV/bin/python" ]]; then
  rm -rf "$FAST_VENV"
  mkdir -p "$(dirname "$FAST_VENV")" "$PIP_CACHE_DIR"
  "$PYTHON_BIN" -m venv --system-site-packages "$FAST_VENV"
fi
ln -sfn "$FAST_VENV" "$COMFY/.venv"

VENV_PY="$FAST_VENV/bin/python"
REQ_HASH="$(sha256sum "$COMFY/requirements.txt" | awk '{print $1}')"
REQ_MARKER="$FAST_VENV/.comfy-requirements-${REQ_HASH}.ok"

if [[ -f "$REQ_MARKER" ]]; then
  echo "  [SKIP verified] Python requirements"
else
  # Reuse the CUDA PyTorch already supplied by the RunPod image. pip sees the
  # system packages through --system-site-packages and installs only missing
  # requirements onto the fast local overlay. --no-compile avoids thousands of
  # unnecessary bytecode writes during setup.
...
if not torch.cuda.is_available():
    raise SystemExit("CUDA PyTorch is not available in the FAST venv")
print(f"  PyTorch {torch.__version__}; CUDA {torch.version.cuda}; {torch.cuda.get_device_name(0)}")
PY

green "  ✓ FAST local venv ready: $FAST_VENV"
...
echo
echo "================================================================="
green " SETUP #5 COMPLETE v4 FAST — GENERATION READY"
echo "================================================================="
echo "ComfyUI       : $COMFY"
echo "ComfyUI tag   : $COMFY_TAG"
echo "Port          : $PORT"
echo "VRAM profile  : Dynamic VRAM ON / reserve 4 GB / cache-none"
echo "Python env    : $FAST_VENV (container-local FAST mode)"
