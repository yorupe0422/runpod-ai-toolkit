...
#
# Target:
#   /workspace/runpod-slim/ComfyUI-Qwen2511
#   port 8188
#
# Designed for a fresh RunPod or persistent storage containing other ComfyUI installs.
# IMPORTANT: Existing /workspace/runpod-slim/ComfyUI is NEVER deleted or modified.
# ============================================================

ROOT="${ROOT:-/workspace/runpod-slim}"
COMFY="$ROOT/ComfyUI-Qwen2511"
VENV="$COMFY/.venv"
PORT="${PORT:-8188}"
...
if [[ ! -d "$COMFY/.git" ]]; then
  if [[ -e "$COMFY" ]]; then
    ...
    warn "Non-git directory already exists at $COMFY"
    warn "Moving it aside instead of deleting it."
    mv "$COMFY" "${COMFY}_old_${TS}" || die "Could not move old Qwen directory aside."
  fi
...
#!/usr/bin/env bash
set -Eeuo pipefail
COMFY="/workspace/runpod-slim/ComfyUI-Qwen2511"
PORT="${PORT:-8188}"
mkdir -p /workspace/runpod-slim/logs

# Kill only an older ComfyUI process bound to this setup, if present.
pkill -f "/workspace/runpod-slim/ComfyUI-Qwen2511/main.py.*--port ${PORT}" 2>/dev/null || true
sleep 2
