...
###############################################################################
# RunPod / ComfyUI / Z-Image Base + Haruka041 Uncut setup v5-fast
#
...
LOCAL_VENV_DIR="${LOCAL_VENV_DIR:-/tmp/comfyui-zimage-base-v5-venv}"
VENV_LINK="${COMFY_DIR}/.venv"
PIP_CACHE_DIR="${PIP_CACHE_DIR:-/tmp/pip-cache-zimage-base-v5}"
STATE_DIR="${BASE_DIR}/.zimage_base_uncut_v5"
...
write_workflows() {
  ...
  curl -fL --retry 8 --retry-all-errors --connect-timeout 20 \
    "${OFFICIAL_WF_URL}" -o "${wf_dir}/ZImage_Base_Official_v5.json"
  python3 - "${wf_dir}/ZImage_Base_Uncut_Haruka041_v5.json" <<'PY'
import json, sys
out=sys.argv[1]
def props(name):
...
Workflows:
  ${COMFY_DIR}/user/default/workflows/ZImage_Base_Uncut_Haruka041_v5.json
  ${COMFY_DIR}/user/default/workflows/ZImage_Base_Official_v5.json
...
worker_main() {
  ...
  if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
    echo "[FATAL] Another v5 Z-Image Base setup worker is active."
    return 1
  fi
